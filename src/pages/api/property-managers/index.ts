...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getPropertyManagersPublic();
    }
  ...
  async function getPropertyManagersPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'property_manager');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.property_manager.count(countOptions as unknown),
      prisma.property_manager.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getPropertyManagers() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.property_manager
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'property_manager'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createPropertyManager() {
    ...
    const data = await prisma.property_manager.create({
      data: body,
    });
