...
import { getPropertyManagers } from 'apiSdk/property-managers';
import { PropertyManagerInterface } from 'interfaces/property-manager';

import { PropertyManagerListPage as ShowCaseList } from 'pages/property-managers';
...
function DashboardPage() {
  ...
  const businessOwnerStat = useSWR<PaginatedInterface<BusinessOwnerInterface>>(
    () => `/business-owners`,
    () => getBusinessOwners({ limit: 1, offset: 0 }),
  );

  const propertyManagerStat = useSWR<PaginatedInterface<PropertyManagerInterface>>(
    () => `/property-managers`,
    () => getPropertyManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: businessOwnerStat?.data?.totalCount,
      isLoading: businessOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Property Managers',
      result: propertyManagerStat?.data?.totalCount,
      isLoading: propertyManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Property Managers
                </Text>
                <NextLink href={`property-managers`} passHref legacyBehavior>
                  <Button
